...
for cap in model-config council health detect dispatch env; do
    if ! octo_provider_has_capability commandcode "$cap"; then test_fail "missing capability $cap"; exit 0; fi
done
test_pass
...
overridden=$(OCTOPUS_COUNCIL_DEFAULT_PROVIDERS="commandcode,claude" bash -c 'source "'$PROJECT_ROOT'/scripts/lib/council.sh"; printf "%s" "$COUNCIL_DEFAULT_PROVIDERS"')
