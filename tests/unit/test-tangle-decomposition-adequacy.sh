...
octo_fallback_chain_agent_specs() {
    ...
    printf '%s\n' 'codex:luna-test' 'codex:sol-test' 'claude:opus-test'
}
...
if [[ "$(cat "$ADEQUACY_COUNT_FILE")" -eq 2 ]] && [[ "$(cat "$RECONSIDER_COUNT_FILE")" -eq 2 ]] && [[ -s "$SPAWN_FILE" ]] && \
   [[ "$first_attempt" == 'agy|researcher|tangle' ]] && [[ "$second_attempt" == 'codex:luna-test|researcher|tangle' ]] && \
...
if [[ "$(cat "$ADEQUACY_COUNT_FILE")" -eq 2 ]] && [[ "$(cat "$RECONSIDER_COUNT_FILE")" -eq 3 ]] && [[ -s "$SPAWN_FILE" ]] && \
   ...
   [[ "${attempts[1]:-}" == 'codex:luna-test|researcher|tangle' ]] && \
   [[ "${attempts[2]:-}" == 'codex:sol-test|researcher|tangle' ]] && \
   [[ "${#attempts[@]}" -eq 3 ]]; then
    test_pass
else
...
if [[ "$status" -ne 0 ]] && [[ "$(cat "$RECONSIDER_COUNT_FILE")" -eq 4 ]] && [[ ! -s "$SPAWN_FILE" ]] && \
   ...
   [[ "${attempts[1]:-}" == 'codex:luna-test|researcher|tangle' ]] && \
   [[ "${attempts[2]:-}" == 'codex:sol-test|researcher|tangle' ]] && \
   [[ "${attempts[3]:-}" == 'claude:opus-test|researcher|tangle' ]] && \
